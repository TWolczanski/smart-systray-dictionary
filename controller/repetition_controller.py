...
class RepetitionController(QObject):
    ...
    def add_meanings_(self, words, definition):
        """Assign the definition to the given words.
        This function adds the words and the definition to the database if they haven't already been added.
        """
        session = Session()
        try:
    ...
    def create_quiz_(self):
        """Create a quiz based on the words that have been added to the database.
        Quizzes generated by this function are random,
        but meanings with the lowest knowledge level are prioritized.
        When the knowledge level is 1 or 2, the quiz has a form of a test with 4 options to choose from
        (note that more than one may be correct).
        In a level 1 quiz you are supposed to choose a definition matching the given word
        and in a level 2 quiz you have to choose a word matching the given definition.
        When the knowledge level is 3 (which is the maximum), in the quiz you are supposed to enter a word
        that matches the given definition (there may be many correct answers).
        """
        session = Session()
        try:
    ...
    def check_quiz_answer_(self, answer):
        ...
        When it is, the knowledge level of the meaning corresponding to the answer is increased
        (on levels 1 and 2) and when it's not, the knowledge level of meanings
        corresponding to every correct answer is decreased (on levels 2 and 3).
        """
        session = Session()
        try:
